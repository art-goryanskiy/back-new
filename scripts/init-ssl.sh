...
if [ -z "$DOMAIN" ]; then
  echo "Укажите DOMAIN (например: export DOMAIN=www.standart82.ru)"
  exit 1
fi

mkdir -p nginx/conf.d
# non-www: убираем префикс www. (www.standart82.ru → standart82.ru)
NON_WWW="${DOMAIN#www.}"
if [ -z "$NON_WWW" ]; then NON_WWW="standart82.ru"; fi
