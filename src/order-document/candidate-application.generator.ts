...
export class CandidateApplicationGenerator {
  ...
  private drawOneCandidateForm(
    ...
    doc.text(`От: ${fioGenitive(learner)}`, MARGIN_LEFT, y, { width: CONTENT_WIDTH, align: 'right' });
    y = doc.y + SECTION_GAP;
    ...
    y = doc.y + 6;
    doc.fontSize(FONT_SIZE);
    doc.text('Прошу принять меня на обучение по программе:', MARGIN_LEFT, y, { width: CONTENT_WIDTH, align: 'justify' });
    y = doc.y + 4;
    const programLine = programTitle
      ? `«${programTitle.replace(/^«|»$/g, '')}»${hours ? ` (${hours} ч.)` : ''}`
      : '____________________________________________________________';
    doc.text(programLine, MARGIN_LEFT, y, { width: CONTENT_WIDTH, align: 'justify' });
    y = doc.y + SECTION_GAP;
    ...
    const consentIntro =
      ...
      (learner ? str(learner.passportRegistrationAddress) : '_________________________') +
