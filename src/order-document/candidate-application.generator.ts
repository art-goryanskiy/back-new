...
/** ФИО в родительном падеже (кого?): Горянский Артем Юрьевич → Горянского Артема Юрьевича */
function fioGenitive(l: OrderLineLearner | null): string {
  if (!l) return '—';
  const last = declineLastNameGenitive(l.lastName ?? '');
  const first = declineFirstNameGenitive(l.firstName ?? '');
  const middle = declinePatronymicGenitive(l.middleName ?? '');
  return [last, first, middle].filter(Boolean).join(' ') || '—';
}
function declineLastNameGenitive(s: string): string {
  const t = s.trim();
  if (!t) return '';
  if (t.endsWith('ский')) return t.slice(0, -4) + 'ского';
  if (t.endsWith('ой')) return t.slice(0, -2) + 'ого';
  if (t.endsWith('ий')) return t.slice(0, -2) + 'ого';
  if (t.endsWith('ый')) return t.slice(0, -2) + 'ого';
  if (t.endsWith('ов')) return t.slice(0, -2) + 'ова';
  if (t.endsWith('ев')) return t.slice(0, -2) + 'ева';
  if (t.endsWith('ин')) return t.slice(0, -2) + 'ина';
  return t;
}
function declineFirstNameGenitive(s: string): string {
  const t = s.trim();
  if (!t) return '';
  if (t.endsWith('ей')) return t.slice(0, -2) + 'ея';
  if (t.endsWith('ий')) return t.slice(0, -2) + 'ия';
  if (t.endsWith('ай')) return t.slice(0, -2) + 'ая';
  if (t.endsWith('ь')) return t.slice(0, -1) + 'я';
  if (t.endsWith('й')) return t.slice(0, -1) + 'я';
  if (t.endsWith('ем')) return t.slice(0, -2) + 'ема'; // Артем → Артема
  if (t.endsWith('ел')) return t + 'а';
  if (/[аеёиоуыэюя]$/i.test(t)) return t;
  return t + 'а';
}
function declinePatronymicGenitive(s: string): string {
  const t = s.trim();
  if (!t) return '';
  if (t.endsWith('ович')) return t.slice(0, -4) + 'овича';
  if (t.endsWith('евич')) return t.slice(0, -4) + 'евича';
  if (t.endsWith('овна')) return t.slice(0, -4) + 'овны';
  if (t.endsWith('евна')) return t.slice(0, -4) + 'евны';
  return t;
}
...
type LearnerWithProgram = {
  learner: OrderLineLearner;
  programTitle: string;
  hours: number;
};
...
export class CandidateApplicationGenerator {
  ...
  private collectLearnersWithPrograms(order: OrderDoc): LearnerWithProgram[] {
    ...
    for (const line of lines) {
      const programTitle = line.subProgramTitle?.trim()
  ...
  async generatePdf(order: OrderDoc): Promise<Buffer> {
    ...
    const toDraw =
      pairs.length > 0
        ...
        : [
            {
              learner: null as OrderLineLearner | null,
              programTitle: '',
              hours: 0,
            },
          ];
    ...
    return new Promise((resolve, reject) => {
      ...
      for (let i = 0; i < toDraw.length; i++) {
        ...
        this.drawOneCandidateForm(
          doc,
          order,
          toDraw[i].learner,
          toDraw[i].programTitle,
          toDraw[i].hours,
        );
      }
  ...
  private drawOneCandidateForm(
    ...
    y = doc.y + 2;
    doc.text('Сеитвелиеву Руслану Ризаевичу', MARGIN_LEFT, y, {
      width: CONTENT_WIDTH,
      align: 'right',
    });
    y = doc.y + 2;
    ...
    // От: ФИО в родительном падеже, по правому краю
    doc.text(`От: ${fioGenitive(learner)}`, MARGIN_LEFT, y, {
      width: CONTENT_WIDTH,
      align: 'right',
    });
    y = doc.y + SECTION_GAP;

    // Заявление: по программе: «...» (N ч.)
    doc.fontSize(12);
    doc.text('Заявление', MARGIN_LEFT, y, {
      width: CONTENT_WIDTH,
      align: 'center',
    });
    y = doc.y + 6;
    doc.fontSize(FONT_SIZE);
    doc.text('Прошу принять меня на обучение по программе:', MARGIN_LEFT, y, {
      width: CONTENT_WIDTH,
      align: 'justify',
    });
    y = doc.y + 4;
    const programLine = programTitle
      ? `«${programTitle.replace(/^«|»$/g, '')}»${hours ? ` (${hours} ч.)` : ''}`
      : '____________________________________________________________';
    doc.text(programLine, MARGIN_LEFT, y, {
      width: CONTENT_WIDTH,
      align: 'justify',
    });
    y = doc.y + SECTION_GAP;
    ...
    const drawTableRow = (num: number, label: string, value: string) => {
      const valueH = doc.heightOfString(value, {
        width: colValueW - cellPad * 2,
      });
      const labelH = doc.heightOfString(label, {
        width: colLabelW - cellPad * 2,
      });
      const rowH = Math.max(
        20,
        Math.ceil(valueH) + cellPad * 2,
        Math.ceil(labelH) + cellPad * 2,
      );
      ensureSpace(rowH);
      const rowY = y;
      ...
      doc.text(
        String(num),
        tableLeft + cellPad,
        rowY + (rowH - FONT_SIZE) / 2,
        { width: colNoW - cellPad * 2 },
      );
      doc.text(label, tableLeft + colNoW + cellPad, rowY + cellPad, {
        width: colLabelW - cellPad * 2,
      });
      doc.text(
        value,
        tableLeft + colNoW + colLabelW + cellPad,
        rowY + cellPad,
        { width: colValueW - cellPad * 2 },
      );
      y = rowY + rowH;
    ...
    drawTableRow(
      2,
      'Дата рождения',
      learner ? formatDate(learner.dateOfBirth) : '—',
    );
    drawTableRow(3, 'Место рождения', '—');
    drawTableRow(4, 'Гражданство', learner ? str(learner.citizenship) : '—');
    const passportStr = learner
      ? [
          learner.passportSeries,
          learner.passportNumber,
          learner.passportIssuedBy,
          formatDate(learner.passportIssuedAt),
        ]
          .filter(Boolean)
          .join(', ') || '—'
      : '—';
    ...
    drawTableRow(
      ...
      learner ? str(learner.passportRegistrationAddress) : '—',
    );
    drawTableRow(
      7,
      'Фактический адрес проживания',
      learner ? str(learner.residentialAddress) : '—',
    );
    const educStr = learner
      ? [
          str(learner.educationQualification),
          formatDate(learner.educationDocumentIssuedAt),
        ]
          .filter((s) => s !== '—')
          .join(', ') || '—'
      : '—';
    drawTableRow(
      8,
      'Образование (квалификация), дата выдачи документа',
      educStr,
    );
    const workStr = learner
      ? [str(learner.workPlaceName), str(learner.position)]
          .filter((s) => s !== '—')
          .join(', ') || '—'
      : '—';
    ...
    // С Уставом... Статья 14 с склонением языка. Каждый абзац с отступом, выравнивание по ширине.
    ...
    y = doc.y + 10;
    doc.text(
      `Подпись _______________ (${learner ? fio(learner) : '___________________'})    Дата заполнения: ${docDateStr} г.`,
      MARGIN_LEFT,
      y,
      { width: CONTENT_WIDTH, align: 'justify' },
    );
    ...
    // Согласие на обработку персональных данных — заголовок по центру
    ensureSpace(50);
    doc.fontSize(10);
    doc.text('СОГЛАСИЕ НА ОБРАБОТКУ ПЕРСОНАЛЬНЫХ ДАННЫХ', MARGIN_LEFT, y, {
      width: CONTENT_WIDTH,
      align: 'center',
    });
    y = doc.y + 6;
    ...
    y = doc.y + 10;
    doc.text(
      `${docDateStr} г. ________________/ ${learner ? fio(learner) : '___________________________'}`,
      MARGIN_LEFT,
      y,
      { width: CONTENT_WIDTH, align: 'justify' },
    );
